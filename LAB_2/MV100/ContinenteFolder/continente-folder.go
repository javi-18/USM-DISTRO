...
func enviarElemento(client pb.ComunicacionClient, elemento Digimon) {
	...
	fmt.Printf("[CONTINENTE-FOLDER] Estado enviado: Nombre %s, Estado: %s", elemento.Nombre, elemento.Estado)
}
...
func main() {
	...
	enviar := digimons[(len(digimons) / 3):(len(digimons) * 2 / 3)]
